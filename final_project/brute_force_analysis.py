...
def corr_cont_cont(df: DataFrame, cont_cont: DataFrame, plot=True) -> DataFrame:
    ...
    for i in cont_cont:
        ...
        if plot:
            # print(y,x)
            tvalue, pvalue, url = linear_regression_scores(y, x, "pred_pred")
            score = {
                "Predictor 1": i[0],
                "Predictor 2": i[1],
                "Pearson's r": score,
...
def corr_catg_catg(df: DataFrame, catg_catg: DataFrame, plot=True) -> DataFrame:
    ...
    for i in catg_catg:
        ...
        if plot:
            url = cat_response_cat_predictor(x, y, "pred_pred")
            score = {
                "Predictor 1": i[0],
                "Predictor 2": i[1],
                "Cramer's V": corr,
...
def corr_catg_cont(df: DataFrame, catg_cont: DataFrame, plot=True) -> DataFrame:
    ...
    for i in catg_cont:
        ...
        if plot:
            url = cont_resp_cat_predictor(response=y, predictor=x, type="pred_pred")
            score = {
                "Predictor 1": i[0],
                "Predictor 2": i[1],
...
def do_analysis(df: DataFrame, predictors: list, response: str) -> list:
    ...
    if continuous_predictors.shape[1] > 1:
        ...
        generate_html(cont_cont_df, click_dict, "cont_cont_corr_table")
        generate_html(cont_cont_mean_df, click_dict, "cont_cont_mean_table")
        ...
        cont_cont_mean_df = pd.DataFrame()
    ...
    if categorical_predictors.shape[1] > 1:
        ...
        generate_html(catg_catg_df, click_dict, "catg_catg_corr_table")
        generate_html(catg_catg_mean_df, click_dict, "catg_catg_mean_table")
        ...
        catg_catg_df = pd.DataFrame()
        catg_catg_mean_df = pd.DataFrame()
    ...
    if categorical_predictors.shape[1] > 0 and continuous_predictors.shape[1] > 0:
        ...
        generate_html(catg_cont_df, click_dict, "catg_cont_corr_table")
        generate_html(catg_cont_mean_df, click_dict, "catg_cont_mean_table")
        ...
        print(
            ...
            "predictors is less than two."
        )
        catg_cont_df = pd.DataFrame()
        catg_cont_mean_df = pd.DataFrame()
